...
class MessagesController extends Controller
{
    ...
    public function index( $id = null)
    {
        ...
        if($pretestUser == null && Auth::user()->role == 'siswa'){
...
//            'user_avatar' => asset('/storage/' . config('chatify.user_avatar.folder') . '/' . $fetch->avatar),
    ...
    public function send(Request $request)
    {
        ...
        if (!$error->status) {
            // send to database
            $checkMessageFirst = ChMessage::where('from_id', Auth::id())->get();
...
//
            $chatAutomatic = ChatAutomatic::inRandomOrder()->first();

            if(count($checkMessageFirst) == 0 && Auth::user()->role == "siswa"){
                $plusMessage = Str::substr($messageID, 10, 1);
                $messageID2 = Str::substr($messageID, 0, 8)."9".((int)$plusMessage + 1);
                Chatify::newMessage([
                    'id' => $messageID2,
                    'type' => $request['type'],
                    'from_id' => $request['id'],
                    'to_id' => Auth::user()->id,
                    'body' => $chatAutomatic->chat,
                    'attachment' => null,
                ]);
            }
        }
    ...
    public function getStudents(Request $request)
    {
        ...
        $contacts = '<p class="message-hint center-el"><span>Belum ada siswa di kelas anda</span></p>';
        $users = User::where([['role', 'siswa'], ['kelas', $kelas], ['id','!=',Auth::user()->id]])->get();
        ...
        if ($users->count() > 0) {
            ...
            foreach ($users as $key => $data){
                $url = asset($data->foto);
                $contacts .= '
                <table class="messenger-list-item " data-contact="'.$data->id.'">
                    <tr data-action="0">
                        <td style="position: relative">
                            <div class="avatar av-m" style="background-image: url('.$url.');">
                            </div>
                        </td>
                        <td>
                            <p data-id="user_'.$data->id.'">
                                '.substr($data->name, 0, 12).' ..
                            </p>
                        </td>
                    </tr>
                </table>
    ...
    public function getTeachers(Request $request)
    {
        // get all users that received/sent message from/to [Auth user]
        $kelas = Auth::user()->kelas;
        $contacts = '<p class="message-hint center-el"><span>Belum ada konselor di kelas anda</span></p>';
        $users = User::where([['role', 'guru'], ['kelas', $kelas], ['id','!=',Auth::user()->id]])->get();
    ...
    public function search(Request $request)
    {
        ...
        $kelas = Auth::user()->kelas;
        if($role == "guru"){
            $records = User::where([['name', 'LIKE', "%{$input}%"], ['kelas', $kelas]]);
        }else if($role == "siswa"){
            $records = User::where([['name', 'LIKE', "%{$input}%"], ['role', 'guru'], ['kelas', $kelas]]);
        }else{
        ...
        return Response::json([
            'records' => $records->count() > 0
                ? $getRecords
                : '<p class="message-hint center-el"><span>Tidak ada yang ditampilkan.</span></p>',
            'addData' => 'html'
        ], 200);
    }
